...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Ambil nilai status_periksa dari form
    $status_periksa = isset($_POST['status_periksa']) ? $_POST['status_periksa'] : 'Belum diperiksa';

    // Simpan data ke dalam tabel 'periksa'
    $tgl_periksa = date("Y-m-d"); // Menggunakan tanggal hari ini
    $waktu_periksa = date("H:i:s"); // Menggunakan waktu saat ini (jam, menit, dan detik)
    
    $query_insert_periksa = "INSERT INTO periksa (tgl_periksa, waktu_periksa, id_daftar_poli, catatan, biaya_periksa, status_periksa) 
    VALUES ('$tgl_periksa', '$waktu_periksa', '$id_daftar_poli', '$catatan', '150000', '$status_periksa')"; // Biaya periksa sebesar Rp 150.000
    
    if ($mysqli->query($query_insert_periksa)) {
        $id_periksa = $mysqli->insert_id; // Mendapatkan ID dari data yang baru saja dimasukkan ke tabel 'periksa'

        // Simpan data obat yang dipilih ke dalam tabel 'detail_periksa'
        if (isset($_POST['obat'])) {
            $obat = $_POST['obat'];
            foreach ($obat as $id_obat) {
                $query_insert_detail = "INSERT INTO detail_periksa (id_periksa, id_obat) 
                VALUES ('$id_periksa', '$id_obat')";
                $mysqli->query($query_insert_detail);
            }
        }

        echo "Data berhasil disimpan ke tabel 'periksa'";
    } else {
            ...
            <div class="col-md-6">
                <div class="card">
                    <div class="card-header">
                        <h3 class="text-center">Form Periksa Pasien</h3>
                    </div>
                    <div class="card-body">
                        <form
                            ...
                            <div class="mb-3">
                                <label for="keluhan" class="form-label">Catatan</label>
